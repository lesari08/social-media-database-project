...
<div>
    ...
    <%  if(request.getAttribute("err") != null) 
        {   
    %>
        <BR> ** <%= request.getAttribute("err") %> ** <BR>
        
    <%  } 
        else if(request.getAttribute("okay") != null) 
        {
    %>
     <BR> ** <%= request.getAttribute("okay") %> **  <BR>
    <%
        }
    %>
	...
	<tr>
		<th>Follower </th>
		<th>Being Followed</th>
	

	</tr>
<c:forEach items="${FollowList}" var="follow"  varStatus="loop">
	<tr>
		<td>${follow.follower}</td>
		<td>${follow.being_followed }</td>
	</tr>
...
<div>		
		   ...
		   %>
		      <form name="asn" method="post" action="AssignThree">
			    <input type="submit"name="button" value="Click to Assign Papers to PC Members for review" />
		      </form>
		      <BR>
		   <%
		     }
         ...
         <form action="editTableUser.jsp">
              <br>To edit: Choose an option from the drop-down menu, then click submit
                <br>
                <br>
